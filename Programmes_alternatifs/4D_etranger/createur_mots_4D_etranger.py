...
def createur_mots_4D(n, Mat, ensemble_mots_existants, langue) : # O(max(n*len(ensemble_mots), n²))
    if langue == 'de' :
    ...
    while i < n :# O(n*(len(ensemble_mots) + n)) amorti
        ...
        if not (mot[:-1] in ensemble_mots_existants or mot in Mots) : # test : O(len(ensemble_mot) + n)
